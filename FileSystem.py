...
class ListWidget(QListWidget):
    ...
    def close_edit(self, *_) -> None:
        if self.edited_item:
            ...
            if sameName:
                while True:
                    new_name, ok = QInputDialog.getText(self, '文件/文件夹已重名', '文件/文件夹已重名，请重新命名:')
                    if ok:
                        if new_name and not any(child.name == new_name for child in self.curNode.children):
                            self.edited_item.setText(new_name)
                            break
                        else:
                            QMessageBox.warning(self, '重名错误', '文件/文件夹名已存在，请重新输入!')
                    else:
                        return
...
class FileSystem(QMainWindow):
    ...
    def init_ui(self):
        def set_window_features():
            # 设置窗口的大小、标题、图标
            self.resize(1200, 800)
            self.setWindowTitle('LINCOLN-lcl的仿Win文件系统')
